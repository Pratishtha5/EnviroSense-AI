...
def get_last_saved_time():
    with db_utils.get_engine().connect() as conn:
        result = conn.execute(text("SELECT MAX(time) FROM member3_features"))
...
while True:
    ...
    print("✅ Sensor LIVE" if status == "LIVE" else "⚠️ Sensor OFF")
    ...
    # CLUSTERING
    # =========================
    X = StandardScaler().fit_transform(df[bins])
    ...
    labels_map = {
        sorted_clusters.index[0]: "Dust",
        sorted_clusters.index[1]: "Traffic",
        sorted_clusters.index[2]: "Smoke"
    }
    ...
    if not features_df.empty:
        inserted = db_utils.save_dataframe(features_df, "member3_features")
    ...
    # PLOT CLUSTERS
    # =========================
    colors = {"Smoke": "red", "Dust": "brown", "Traffic": "blue"}
